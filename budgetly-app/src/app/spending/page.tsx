...
const navItems = [
  ...
  { label: "Spending", icon: "card", active: true },
...
const recurringCurrent = [
  { name: "Netflix Premium", due: "Aug 15", amount: 19.99 },
  { name: "Spotify Family", due: "Aug 20", amount: 16.99 },
  { name: "Gym Membership", due: "Aug 01", amount: 45.0 },
  { name: "Cloud Storage", due: "Sep 05", amount: 9.99 },
];

const recurringOlder = [
  { name: "Magazine Subscription", lastPaid: "Feb 2023", amount: 12.0 },
  { name: "Online Course", lastPaid: "Jan 2023", amount: 29.99 },
];
...
export default function SpendingPage() {
  const maxValue = Math.max(...monthlySpending.map((item) => item.value));
    ...
    <div className={styles.page}>
      ...
      <main className={styles.main}>
        <section className={styles.grid}>
            ...
            <article className={styles.card}>
              ...
              <div className={styles.chart}>
                <div className={styles.chartGrid}>
                  {[620, 465, 310, 155].map((value) => (
                    <div key={value} className={styles.chartRow}>
                      <span>{value}</span>
                      <span />
                    </div>
                  ))}
          ...
          <article className={`${styles.card} ${styles.recurring}`}>
            <h2>Recurring Payments</h2>
            <div className={styles.recurringSection}>
              <h3>Current</h3>
              <ul>
                {recurringCurrent.map((item) => (
                  <li key={item.name}>
                    <div>
                      <span>{item.name}</span>
                      <small>Next due: {item.due}</small>
                    </div>
            ...
            <div className={styles.recurringSection}>
              <h3>Older</h3>
              <ul>
                {recurringOlder.map((item) => (
                  <li key={item.name}>
                    <div>
                      <span>{item.name}</span>
                      <small>Last paid: {item.lastPaid}</small>
                    </div>
                    <strong>{formatCurrency(item.amount)}</strong>
                  </li>
                ))}
